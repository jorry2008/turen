...
<div class="row cms-img-form">
    <div class="col-md-12">
        ...
        <?= $form->field($model, 'column_id')->dropDownList(ArrayHelper::map(Column::find()->where(['type'=>Column::CMS_TYPE_IMG])->alive()->all(), 'id', 'name')) ?>
...
// 			'template' => '{label} <div class="row"><div class="col-sm-4">{input}{error}{hint}</div></div>',
		...
		<?= $form->field($model, 'pic_url')->hint('<i class="fa fa-info-circle"></i> '.Yii::t('fileinput', 'Note: Limit upload one picture.'))->widget(FileInput::classname(), [
	    	...
	    	'pluginOptions' => [
	    		'uploadUrl' => Url::to(['/cms/img/file-upload']),
	    		'uploadAsync' => true,//异步上传
	    		'initialPreview' => General::showImages($model->pic_url),
	    		'initialPreviewConfig' => General::showLinks($model->pic_url, 'pic_url', 'img', '/cms/img/file-upload'),
	    		...
	    		'uploadExtraData' => [
	    			'dir' => 'img',//目录标识，广告
	    			'name' => 'Img[pic_url]',//指定资源获取标识名
	    			'route' => '/cms/img/file-upload',
	    			'field' => 'pic_url',
	    		],
	    	],
		]) ?>
		...
		<?= $form->field($model, 'picarr')->hint('<i class="fa fa-info-circle"></i> '.Yii::t('fileinput', 'Note: Allowed to upload more pictures.'))->widget(FileInput::classname(), [
	    	...
	    	'pluginOptions' => [
	    		...
	    		'initialPreviewConfig' => General::showLinks($model->picarr, 'picarr', 'img', '/cms/img/file-upload'),
	    		'previewFileType' => 'any',//预览所有类型文件
	    		//'initialCaption'=>"原有的图片",
	    		'overwriteInitial'=>true,//直接覆盖原有的图片
	    		'maxFileSize' => Yii::$app->params['config']['config_pic_size'],//限制大小
